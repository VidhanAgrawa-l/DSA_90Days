...
int binarySearch(int arr[], int size, int key) {
    ...
    while(start <= end) {
        ...
        if(key > arr[mid]) {             //else if bhi use kr skte hai
            // Go to right part
            start = mid + 1;
        } else {
            // Go to left part
        ...
        mid = start + (end-start)/2;    // loop ke baad mid update bhi toh krna padega taki,next ietration of binary seacrch kr ske
    }
